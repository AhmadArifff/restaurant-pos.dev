...
export default function CustomerPaymentPanel({ order, onOrderUpdate, compact = false }) {
  ...
  return (
    <div className={`mt-5 rounded-3xl border border-sky-300/20 bg-sky-400/10 ${compact ? 'p-4' : 'p-5'} text-sky-50`}>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="rounded-2xl border border-sky-200/20 bg-[#0D0A06]/45 px-4 py-3">
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[160px_1fr]">
        {method.qr_image_url ? (
          <img
            ...
            alt={method.name}
            className="aspect-square w-full max-w-[180px] rounded-2xl border border-sky-100/20 bg-white object-contain p-2"
          />
        ) : (
          <div className="grid aspect-square max-w-[180px] place-items-center rounded-2xl border border-sky-100/20 bg-[#0D0A06]/45 text-center text-xs text-sky-100/55">
            QR tidak tersedia
          </div>
        )}
      ...
      {isPaid ? (
        ...
        <div className="mt-4 rounded-2xl border border-emerald-300/25 bg-emerald-500/15 p-3 text-sm text-emerald-100">
          ...
          {order.payment_proof_url && (
            <a href={resolveAssetUrl(order.payment_proof_url)} target="_blank" rel="noreferrer" className="mt-2 block font-black underline">
